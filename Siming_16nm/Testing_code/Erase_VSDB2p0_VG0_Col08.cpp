...
//      E3646A_SetVoltage_CurrentLimit(_VDD_IO_V_NIDAQ, 2, 5.0, 0.02); //V_NIDAQ=5V, 10mA limit 
	...
	printf("I_VSPARE = %f\n", 0.001*E3646A_MeasAvgCurrent(_VSPARE_VAB, 1));
...
//	scan("../Scan_files/NIDAQ_test_data", 1, 1000.0); 
	...
	for (col = 8; col < 9; col = col + 1){
		...
		for (PulseCycle = 1; PulseCycle <= 3; PulseCycle++){
			sprintf(Measure_file, "../Data/Chip%02d_Col%02d_erase_VSDB_tunneling_VAsource_VBdrain_%02d", chip, col, PulseCycle);
			erase_VSDB_tunneling(Measure_file, VSDB, VDD_DIG_WL, "40ms", chip, col, 0, Num_of_Pulse);

			sprintf(Measure_file, "../Data/Erase_Chip%02d_Col%02d_Ids_Vgs_VAsource_VBdrain_%02d", chip, col, PulseCycle);
			IDS_VGS(Measure_file, col, chip, 0);
			sprintf(Measure_file, "../Data/Erase_Chip%02d_Col%02d_Ids_Vgs_VAdrain_VBsource_%02d", chip, col, PulseCycle);
			IDS_VGS(Measure_file, col, chip, 1);
		}
	}
